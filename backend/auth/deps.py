...
async def get_current_user(
    ...
    username_ok = (
        secrets.compare_digest(credentials.username, user.username) if user else False
    )
    password_ok = (
        verify_password(credentials.password, user.hashed_password) if user else False
    )
